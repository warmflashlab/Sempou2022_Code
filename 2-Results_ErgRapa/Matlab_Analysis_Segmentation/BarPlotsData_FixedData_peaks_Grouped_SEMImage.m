...
function BarPlotsData_FixedData_peaks_Grouped_SEMImage(blackBG)
...
AllmeanData = zeros(analysisParam.nChannels,3);
...
load(['Plate1_Well',num2str(contoplot(daynum)),'_peaks_BGNorm'],'peaks');
meanaux=zeros(1,3);
for imagenum =1:3
    meanaux(imagenum) = mean(peaks{imagenum}(:,5+channelnum)./peaks{imagenum}(:,5));
end


AllmeanData(channelnum,daynum) = mean(meanaux); 
AllsemData(channelnum,daynum) = std(meanaux)/sqrt(3);    
...
saveas(fig,[analysisParam.figDir filesep 'GroupedBarPlots-SEMImages-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4)],'svg')
saveas(fig,[analysisParam.figDir filesep 'GroupedBarPlots-SEMImages-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4)],'fig')
...
saveas(fig,[analysisParam.figDir filesep 'GroupedBarPlots-SEMImages-AllGenes-DAPINorm-' analysisParam.dataSegmentation(1:end-4) '.pdf'],'pdf');
% 
