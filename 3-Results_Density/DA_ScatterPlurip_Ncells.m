...
function DA_ScatterPlurip_Ncells(ConditionsSelection,titleplottosave,varargin)
...
stdnCells = [];
    ...
    for condnum = 1:nCon
        ...
        if stdmean
            ...
            nCellsAux = zeros(1,AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(end,end));
            for posnum = 1:AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(end,end)
                ...
                meanDataAux(posnum) = mean(AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(indposnum,2+FindChannelsinConditions(condnum,channelnum)));%.*AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(indposnum,3));
                nCellsAux(posnum) = length(AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(indposnum,2+FindChannelsinConditions(condnum,channelnum)));%.*AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(indposnum,3));
            end
            ...
            nCells = [nCells,meannonan(nCellsAux)];
            stdnCells = [stdnCells,stdnonan(nCellsAux)];
        else
                    
            meanData = [meanData,meannonan(AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(:,2+FindChannelsinConditions(condnum,channelnum)))];%.*AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(:,3))];
            stdData = [stdData,stdnonan(AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(:,2+FindChannelsinConditions(condnum,channelnum)))];%.*AllDataExperiment{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(:,3))];
            
        end
        

        daystickslabels{condnum} = analysisParam.NamesConditions{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)};
        %nCells = [nCells, str2num(analysisParam.NamesConditions{ConditionsSelection(1,condnum)}{ConditionsSelection(2,condnum)}(1))*1000];
        %stdnCells = [stdnCells,0];
    ...
    errorbar(nCells(ii),meanData(ii),stdData(ii)/2,stdData(ii)/2,stdnCells(ii)/2,stdnCells(ii)/2,'o')
...
%     end
    ...
    if channelnum==1
    legend(daystickslabels,'Location','northoutside','Orientation','horizontal')
    end

    if channelnum==nChan
    xlabel('Final number of cells')
    end
...
saveas(fig,[analysisParam.figDir filesep 'DA_ScatterFinalnCells-',colorbgplotname,'-AllGenes-NoDAPINorm-', titleplottosave, '-', raworcleantitle{raworclean+1}, '.pdf'],'pdf');
