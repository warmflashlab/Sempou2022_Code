function Find_Limits_v2(pathname,savingpathforImages,PlateMaxNum,Channelsnames,ChannelMaxNum,WellsWithData,ImagesperWell,Conditionsnames,OrderChannels,varargin)
%% Description
...
for ChannelNumber = 1:length(ChannelsPresent)
    PlateNumberaux = PlateCoordinates{ChannelNumber}(1);
    WellNumberaux = WellCoordinates{ChannelNumber}(1);
    ChannelCoordinateaux = ChannelCoordinates{ChannelNumber}(1);
    
    DataAux = AllDataExperiment{PlateNumberaux}{WellNumberaux}(:,3+ChannelCoordinateaux);
    
     maxlim = quantile(DataAux,0.995);
     minlim = quantile(DataAux,0.005);
    ...
    for imageswithchannel = 1:length(PlateCoordinates{ChannelNumber})
        ...
        for positionnumber = 1:ImagesperWell
            ...
            image16bit = imread([pathname,'/P',num2str(PlateCoordinates{ChannelNumber}(imageswithchannel)),'_W',num2str(WellCoordinates{ChannelNumber}(imageswithchannel)),'_',num2str(positionnumber),'_MaxProj.tif'],ChannelCoordinates{ChannelNumber}(imageswithchannel));
            
            
            maxlim2 = max([maxlim;quantile(DataPlot,0.995)]);
            minlim2 = min([minlim;quantile(DataPlot,0.005)]);
...
mkdir(['Well',num2str(wellnumber)]) 
        ...
        for positionnumber = 1:ImagesperWell
            ...
            for channelnumber = 1:ChannelMaxNum
              ...
              image16bit = imread([pathname,'/P',num2str(platenumber),'_W',num2str(wellnumber),'_',num2str(positionnumber),'_MaxProj.tif'],channelnumber);
...
save([savingpathforImages,'/limitschannelsimages'],'limitschannels');
